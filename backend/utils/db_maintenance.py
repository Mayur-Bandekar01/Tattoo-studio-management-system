...
def ensure_schema_consistency():
    ...
    try:
        ...
        if not cursor.fetchone():
            cursor.execute(
                "ALTER TABLE artist ADD COLUMN profile_image VARCHAR(255) DEFAULT NULL"
            )
            conn.commit()
        ...
        if not cursor.fetchone():
            cursor.execute(
                "ALTER TABLE inventory ADD COLUMN artist_id VARCHAR(50) DEFAULT NULL"
            )
            cursor.execute(
                "ALTER TABLE inventory ADD CONSTRAINT fk_inv_artist FOREIGN KEY (artist_id) REFERENCES artist(artist_id) ON DELETE CASCADE"
            )
            conn.commit()
