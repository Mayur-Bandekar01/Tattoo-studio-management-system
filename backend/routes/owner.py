...
def owner_dashboard():
    ...
    from ..services.owner_service import get_owner_dashboard_data
    
    conn = get_db()
    with conn.cursor(dictionary=True) as cursor:
        data = get_owner_dashboard_data(cursor)
...
def owner_cancel(appointment_id):
    conn = get_db()
    with conn.cursor() as cursor:
        cursor.execute("SELECT appointment_id FROM appointment WHERE appointment_id = %s", (appointment_id,))
